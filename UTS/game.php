...
if (isset($_POST['submit']) && isset($_SESSION['jawaban'])){
  ...
  elseif ($tebakan != $_SESSION['jawaban']) {
    ...
    if ($_SESSION['lives'] <= 0){
      ...
      $conn = mysqli_connect ("localhost", "root", "", "uts", 3306);
      if ($conn->connect_error){
        die("Koneksi database hall of fame gagal: " . $conn->connect_error);
      } else {
        //Sql query input data
        $score = $_SESSION['score'];
        $query = "INSERT INTO gameuts (nama, score) VALUES ('$nama', '$score')";
        //Sql query run
      ...
      $query = "SELECT nama,score  FROM gameuts ORDER BY Score DESC";
      $result = $conn->query($query);
